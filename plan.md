...
### M6. 안정성/보안 하드닝
- 잘못된 보드 크기/좌표 입력에 대한 방어 로직 추가(패닉 방지)
- 패턴 로드(네트워크) 타임아웃/응답 크기 제한/콘텐츠 타입 검증
- CLI 옵션 유효성 검증(`fps > 0`, 색상 포맷, URL 검증) 및 명확한 오류 메시지 제공
- 입력 상태 플래그의 라이프사이클 정리(일회성 이벤트 소비)
- 대용량/악성 패턴 입력에 대한 파서 회복력 테스트 강화

---

## 현재 진행 상태 (업데이트)
- [x] 현재 마일스톤: **M4 TUI 렌더러/입력** 완료 (E~F 섹션 테스트 완료 기준)
- [x] 현재 마일스톤: **M5 리사이즈/CLI/통합** 완료
- [x] 현재 마일스톤: **M6 안정성/보안 하드닝** 완료 (I01~I09)
- [x] DoD 진행: **DOD-2 (Game of Life 4대 규칙 테스트 검증)** 완료
- [x] DoD 진행: 그 외 항목(DOD-1,4,5,7) 완료
...
- [x] C07 시작 시 초기 활성 셀은 중앙 20x20 범위(기존 대비 2배)에서 50% 밀도로 배치되어야 한다.
...
### G. 리사이즈/CLI
- [x] G01 리사이즈 이벤트 수신 시 보드 크기와 렌더링 영역을 재계산해야 한다.
- [x] G02 리사이즈 후에도 앱이 종료되지 않고 계속 실행되어야 한다.
- [x] G03 `--help`는 사용법/옵션/단축키/URL 예시를 출력해야 한다.
- [x] G04 `--version`은 버전 문자열을 출력해야 한다.
- [x] G05 `--pattern-url` 지정 시 시작 시점에 패턴 로드를 시도해야 한다.

### H. 통합/DoD 검증
- [x] H01 기본 실행(옵션 없음) 시 랜덤 패턴으로 시뮬레이션이 시작되어야 한다.
- [x] H02 핵심 사용자 시나리오(재생→일시정지→재시작→외부패턴 로드)가 실패 없이 동작해야 한다.
- [x] H03 DoD 체크리스트 7개 항목이 모두 충족되어야 한다.

### I. 안정성/보안 하드닝
- [x] I01 보드 크기가 0 이하일 때도 엔진이 패닉 없이 의미 있는 오류를 반환해야 한다.
- [x] I02 잘못된 좌표 접근(`SetAlive`, `IsAlive`)은 패닉 대신 오류/무시 정책으로 처리되어야 한다.
- [x] I03 `NextGeneration`은 0x0 보드에서 모듈로 연산 패닉 없이 안전 종료되어야 한다.
- [x] I04 `--fps`가 0 이하일 때 실행을 중단하고 사용 가능한 범위를 안내해야 한다.
- [x] I05 `--pattern-url`은 시작 시점에 `ValidateWikiURL` 검증을 통과한 값만 허용해야 한다.
- [x] I06 패턴 다운로드는 타임아웃을 설정하고, 제한 크기(예: 1MB) 초과 시 중단해야 한다.
- [x] I07 HTTP 응답이 200이 아니거나 `text/*`가 아니면 복구 가능한 오류를 반환해야 한다.
- [x] I08 RLE 파서가 긴 run-length 숫자/오버플로 입력에서 패닉 없이 오류를 반환해야 한다.
- [x] I09 `l` 키 플래그(`LoadPatternRequested`)는 처리 후 자동으로 초기화되어 재진입 오동작이 없어야 한다.

---

## DoD 추적 체크리스트 (`SPEC.md` §10 매핑)
- [x] DOD-1 기본 랜덤 시뮬레이션 동작
- [x] DOD-2 Game of Life 4대 규칙 테스트 검증
- [x] DOD-3 살아있는/죽은 세포 색상 구분
- [x] DOD-4 resize 자동 반영
- [x] DOD-5 외부 wiki 링크 패턴 로드
- [x] DOD-6 단축키 안내 상시 표시
- [x] DOD-7 `--help` 포함 기본 CLI 명령 동작
...
- 재발 방지 규칙
  - 문서(Milestone/백로그/DoD)는 기능 변경과 같은 PR에서 동일하게 갱신한다.
  - Milestone 진행률을 변경할 때는 체크박스 수치가 먼저 일치하는지 확인한다.
  - 버전/예시 숫자는 한 곳에서만 관리하고, 변경 시 관련 문서·코드/테스트 항목 동시 점검한다.
