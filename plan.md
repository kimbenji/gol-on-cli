...
- [ ] A02 기본 앱 설정 구조체가 기본값(fps, seed 모드, 색상 모드)을 제공해야 한다.

### B. 엔진 규칙 (핵심)
- [ ] B01 죽은 세포는 이웃 3개일 때 살아나야 한다(Birth).
- [ ] B02 살아있는 세포는 이웃 2개일 때 생존해야 한다(Survival-2).
- [ ] B03 살아있는 세포는 이웃 3개일 때 생존해야 한다(Survival-3).
- [ ] B04 살아있는 세포는 이웃 1개일 때 죽어야 한다(Underpopulation).
- [ ] B05 살아있는 세포는 이웃 4개일 때 죽어야 한다(Overpopulation).
- [ ] B06 다음 세대 계산은 현재 세대를 기준으로 동시 갱신되어야 한다.
- [ ] B07 보드 경계는 래핑(toroidal)으로 이웃을 계산해야 한다.

### C. 초기화/루프
- [ ] C01 랜덤 초기화는 지정한 보드 크기를 유지해야 한다.
- [ ] C02 같은 seed면 같은 초기 보드를 생성해야 한다.
- [ ] C03 루프가 1틱 진행하면 세대 수가 1 증가해야 한다.
- [ ] C04 일시정지 상태에서는 틱이 와도 세대가 증가하지 않아야 한다.
- [ ] C05 재개 후에는 다시 세대가 증가해야 한다.
- [ ] C06 재시작(`r`) 시 랜덤 초기화와 함께 세대가 0으로 초기화되어야 한다.
...
- [ ] E02 TrueColor 지원 환경에서는 TrueColor 팔레트를 우선 사용해야 한다.
- [ ] E03 TrueColor 미지원 환경에서는 폴백 색상을 사용해야 한다.
- [ ] E04 상태바에 세대 수/재생 상태/패턴 출처가 표시되어야 한다.
- [ ] E05 단축키 안내(`q`, `h/?`, `space`, `r`, `l`)가 항상 표시되어야 한다.

### F. 입력/상호작용
- [ ] F01 `space` 입력 시 재생/일시정지 상태가 토글되어야 한다.
- [ ] F02 `h` 또는 `?` 입력 시 도움말 표시가 토글되어야 한다.
- [ ] F03 `l` 입력 시 패턴 로딩 플로우가 시작되어야 한다.
- [ ] F04 `q` 입력 시 안전 종료 상태로 전환되어야 한다.
